...
StoreWatcher *startWatcher(JavaVM *pJavaVM, Store *pStore, jobject pLock) {
    ...
    if (pthread_attr_init(&lAttr)) abort();
    if (pthread_create(&watcher->mThread, &lAttr, runWatcher, watcher)) abort();
    return watcher;
}
...
void *runWatcher(void *pArgs) {
    ...
    javaVMAttachArgs.name = "WatcherThread";
    javaVMAttachArgs.group = NULL;

    JavaVM *javaVM = watcher->mJavaVM;
    JNIEnv *pEnv;
    if (javaVM->AttachCurrentThreadAsDaemon(&pEnv, &javaVMAttachArgs) != JNI_OK)
        abort();
    ...
    while (true) {
        ...
        pEnv->MonitorEnter(watcher->mLock);
        ...
        pEnv->MonitorExit(watcher->mLock);
...
void processEntry(StoreEntry *pEntry) {
    StoreValue *value = &pEntry->mValue;
    switch (pEntry->mType) {
        case StoreType_Integer:
            int32_t v = value->mInteger;
            if (v > 10000) {
                value->mInteger = 10000;
            }
